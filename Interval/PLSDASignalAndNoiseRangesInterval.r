...
###### NAME:           PLSDASignalAndNoiseRangesInterval.r
...
for (repetition in repetitionsSeq) {
  ...
  OriginalM = matrix(runif(nSamples*noiseSeq[length(noiseSeq)],0,1), ncol=noiseSeq[length(noiseSeq)]) #To avoid having a new matrix with every execution, we just 
  ...
  for (nNoise in noiseSeq) {
    ...
    for (nSignal in singalSeq) {
      ...
      for (j in seq(1:nSignal)) {
        ...
        # m = cbind(c(runif(nSamples/2,limitsAlsoLeft[j%%4+1],limitsAlsoLeft[j%%4+2]), runif(nSamples/2,0,1)),m)
        m = cbind(c(runif(nSamples/2,limitsAlsoLeft[j%%3+1],limitsAlsoLeft[j%%3+2]), runif(nSamples/2,0,1)),m)

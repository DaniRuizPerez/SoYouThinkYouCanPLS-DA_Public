...
###### NAME:           PLSDASignalAndNoiseRangesIntervalPerformance.r
...
for (repetition in repetitionsSeq) {
  ...
  OriginalM = matrix(runif(samplesSeq[length(samplesSeq)]*noiseSeq[length(noiseSeq)],0,1), ncol=noiseSeq[length(noiseSeq)]) #To avoid having a new matrix with every execution, we just 
  ...
  for (nNoise in noiseSeq) {
    ...
    for (nSamples in samplesSeq) {
      ...
      for (j in seq(1:nSignal)) {
        m = cbind(c(runif(nSamples/2,limitsAlsoLeft[j%%5+1],limitsAlsoLeft[j%%5+2]), runif(nSamples/2,0,1)),m)
